//
// SimpleFrameGenerator.cpp  —  ADAPTED for Avatar Studio Camera
// -----------------------------------------------------------------------------
// Drop-in replacement for the Microsoft VirtualCamera sample's
// SimpleFrameGenerator.cpp. The ONLY behavioural change is in
// _CreateRGB32Frame: instead of painting a moving gray gradient, it copies the
// latest avatar frame out of the shared memory-mapped file (see SharedFrame.h /
// avatar_sharedframe.py). When no avatar is running yet, it paints a calm dark
// "standby" fill so the camera shows something valid rather than garbage.
//
// Everything else (NV12 conversion, color helpers) is unchanged from the sample.
//
#include "pch.h"
    ...
    _In_ ULONG rgbMask)
{
    ...
    // --- AVATAR STUDIO CAMERA: pull the live frame from shared memory ---------
    // One reader for the lifetime of this stream. The avatar process may start
    // AFTER an app opens the camera, so keep retrying Open() until it appears.
    static avatarcam::SharedFrameReader s_reader;
    if (!s_reader.IsOpen())
    {
        s_reader.Open();
    }
    if (s_reader.IsOpen() &&
        s_reader.ReadBGRA(pBuf, len, pitch, width, height))
    {
        return S_OK;   // delivered a real avatar frame
    }

    // --- standby fill (no avatar yet): calm dark slate, faint top-down ramp ---
    for (unsigned int r = 0; r < height; r++)
    {
        uint32_t* p = (uint32_t*)(pBuf + (r * pitch));
        BYTE v = (BYTE)(10 + (r * 18) / (height ? height : 1));
        uint32_t px = ((uint32_t)v << 16) | ((uint32_t)v << 8) | (uint32_t)(v + 6);
...
//////////////////////////////////////////////////
// pixelFormatConverter  (unchanged from the sample)
