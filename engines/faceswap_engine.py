...
class FaceSwapEngine:
    ...
    @staticmethod
    def _nose_off(kps):
        """Head-yaw proxy: nose offset from the eye-centre, in eye-widths. Same
        metric for the operator and each hair view, so they match by head angle."""
        ec = (kps[0] + kps[1]) / 2.0
        ed = float(np.linalg.norm(kps[0] - kps[1])) + 1e-6
        return float((kps[2, 0] - ec[0]) / ed)

    def _load_hair(self, style):
        """Lazy-load a hairstyle's MULTI-VIEW assets: list of (nose_off, rgba, kps),
        one per rendered yaw. Falls back to the single-view folder if present."""
        if not hasattr(self, "_hair_assets"):
            self._hair_assets = {}
        if style in self._hair_assets:
            return self._hair_assets[style]
        views = []
        mvdir = os.path.join(PROJECT_DIR, "hairstyles_mv", style)
        if os.path.isdir(mvdir):
            for png in sorted(glob.glob(os.path.join(mvdir, "v*.png"))):
                npy = png[:-4] + ".npy"
                rgba = cv2.imread(png, cv2.IMREAD_UNCHANGED)
                if rgba is not None and rgba.shape[2] == 4 and os.path.exists(npy):
                    kp = np.load(npy).astype(np.float32)
                    views.append((self._nose_off(kp), rgba, kp))
        else:
            hp = os.path.join(PROJECT_DIR, "hairstyles", f"{style}.png")
            kpf = os.path.join(PROJECT_DIR, "hairstyles", f"{style}.npy")
            if os.path.exists(hp) and os.path.exists(kpf):
                rgba = cv2.imread(hp, cv2.IMREAD_UNCHANGED)
                kp = np.load(kpf).astype(np.float32)
                if rgba is not None and rgba.shape[2] == 4:
                    views.append((self._nose_off(kp), rgba, kp))
        self._hair_assets[style] = views or None
        return self._hair_assets[style]

    def _apply_hairstyle(self, out, kps):
        """Composite the hairstyle: pick the VIEW whose head-angle matches yours
        (so the hair TURNS with your head), align it to your face, smoothed."""
        style = getattr(self, "_hairstyle", "none")
        if style == "none":
            return out
        views = self._load_hair(style)
        if not views:
            return out
        try:
            no = self._nose_off(kps)
            self._hair_no = no if getattr(self, "_hair_no", None) is None \
                else 0.6 * self._hair_no + 0.4 * no          # smooth the yaw estimate
            idx = int(np.argmin([abs(v[0] - self._hair_no) for v in views]))
            _, rgba, akps = views[idx]
            M, _ = cv2.estimateAffinePartial2D(akps, kps.astype(np.float32), method=cv2.LMEDS)
            if M is None:
                return out
            if getattr(self, "_hair_M", None) is not None and self._hair_vi == idx:
                M = 0.55 * M + 0.45 * self._hair_M          # smooth (same view)
            self._hair_M = M; self._hair_vi = idx
            H, W = out.shape[:2]
